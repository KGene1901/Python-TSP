...
def a_star_search(start_city, num_cities, dist_matrix):
    ...
    if num_cities == 1:
        ...
        desired_city = fringe[0]
        while len(tour) != num_cities: # iterates through the next chosen cities
            
            # fringe.remove(desired_city)

            ### Terminate function close to 1 min mark ###
            ...
            heuristic = 0
            isHeuristicsCalculated = False
            fringe = []
            
            for nextPotentialCity in range(len(dist_matrix[desired_city[0]])): # iterates through all neighbours of the current city
                if nextPotentialCity in tour:
                    continue
